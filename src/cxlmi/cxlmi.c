...
send_cmd_cci_mailbox (struct cxlmi_endpoint *ep, struct cxlmi_cci_msg *req_msg, size_t req_msg_sz,
		 struct cxlmi_cci_msg *rsp_msg, size_t rsp_msg_sz,
		 size_t rsp_msg_sz_min)
{
	int rc, len;
	struct cxlmi_ctx *ctx = ep->ctx;
	rc = send_mailbox_command(ep->config_space_mailbox, req_msg->command, &req_msg_sz, req_msg->payload, &rsp_msg->return_code);
	if (rc)
		return -1;
	return 0;
}
		 ...
		 size_t rsp_msg_sz_min)
{
	...
	else if(ep->config_space_mailbox != NULL) {
		rc = send_cmd_cci_mailbox(ep, req_msg, req_msg_sz,
				      rsp_msg, rsp_msg_sz, rsp_msg_sz_min);
	}
	else {
		if (!ti || ti->level == 0)
...
CXLMI_EXPORT struct cxlmi_endpoint * cxlmi_config_space_access(struct cxlmi_ctx *ctx,
						const char *devname)
{
	struct cxlmi_endpoint *ep, *tmp;
	int errno_save;
	...
	ep->config_space_mailbox = mailbox_base_address;
